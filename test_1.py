...
def test_post_v1_account_user_3():
    headers = {
        'accept': '*/*',
...
def test_post_v1_account_login_3():
    headers = {
        'accept': 'text/plain',
        # Already added when you pass json= but not when you pass data=
        # 'Content-Type': 'application/json',
...
def test_get_all_email():
    ...
    print(qson)
    confirmation_link_url = qson['ConfirmationLinkUri']
    token = confirmation_link_url.split('/')[-1]
    print()
    pprint.pprint(token)
    #token = response.json()['items'][0]['Content']['Body']['ConfirmationLinkUri'].split('/')[-1]
    return token
...
def test_post_v1_account_password():
    headers = {
        'accept': 'text/plain',
        'X-Dm-Auth-Token': '364bddc5-b80d-4283-b179-52e7b602560f',
        # Already added when you pass json= but not when you pass data=
        # 'Content-Type': 'application/json',
    }
    ...
    response = requests.post('http://localhost:5051/v1/account/password', headers=headers, json=json_data)
